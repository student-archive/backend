...
/** Router, which handles requests to /page/:id. */
public class PageRouter {

  /** Base path for all requests, which this router handles. */
  public static String BASE_URL = "/page/:id";
  /** Logger for this class. */
  public static Logger logger = LoggerFactory.getLogger(PageRouter.class);

  /**
   * Handles GET request to /page/:id.
   *
   * @param request request object
   * @param response response object
   * @return JSON representation of exact page
   */
  public static String get(Request request, Response response) {
    ...
    try {
      if (dao.findById(UUID.fromString(request.params("id")))) {
        Page page = dao.getById(request.params("id"));
  ...
  /**
   * Handles DELETE request to /page/:id.
   *
   * @param request request object
   * @param response response object
   * @return JSON representation of id of deleted page
   */
  public static String delete(Request request, Response response) {
    ...
    try {
      if (dao.findById(UUID.fromString(request.params("id")))) {
        ...
        logger.info("Page {} deleted", request.params("id"));
  ...
  /**
   * Handles PATCH request to /page/:id.
   *
   * @param request request object
   * @param response response object
   * @return JSON representation of updated page
   */
  public static String patch(Request request, Response response) {
    ...
    try {
      if (dao.findById(UUID.fromString(request.params("id")))) {
        logger.info("Page {} found", request.params("id"));
        Page page = dao.getById(request.params("id"));
        Page body = new Gson().fromJson(request.body(), Page.class);
        body.setId(page.getId());
        Page updated = (Page) ClassUtil.mergeObjects(body, page);
        dao.update(updated);
  ...
  /**
   * Handles PUT request to /page/:id.
   *
   * @param request request object
   * @param response response object
  ...
  /**
   * Handles POST request to /page/:id.
   *
   * @param request request object
   * @param response response object
   * @return JSON representation of 405 error
