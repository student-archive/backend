...
/** Router, which handles requests to /quiz/:id. */
public class QuizRouter {

  /** Base path for all requests, which this router handles. */
  public static String BASE_URL = "/quiz/:id";
  /** Logger for this class. */
  public static Logger logger = LoggerFactory.getLogger(QuizRouter.class);

  /**
   * Handles GET request to /quiz/:id.
   *
  ...
  public static String get(Request request, Response response) {
    ...
    try {
      if (dao.findById(UUID.fromString(request.params("id")))) {
        Quiz quiz = dao.getById(request.params("id"));
  ...
  /**
   * Handles DELETE request to /quiz/:id.
   *
   * @param request request object
   * @param response response object
  ...
  /**
   * Handles PATCH request to /quiz/:id.
   *
   * @param request request object
   * @param response response object
  ...
  public static String patch(Request request, Response response) {
    ...
    try {
      if (dao.findById(UUID.fromString(request.params("id")))) {
        ...
        Quiz quiz = dao.getById(request.params("id"));
        Quiz body = new Gson().fromJson(request.body(), Quiz.class);
        body.setId(quiz.getId());
        Quiz updated = (Quiz) ClassUtil.mergeObjects(body, quiz);
        dao.update(updated);
  ...
  /**
   * Handles PUT request to /quiz/:id.
   *
   * @param request request object
   * @param response response object
  ...
  /**
   * Handles POST request to /quiz/:id.
   *
   * @param request request object
   * @param response response object
   * @return JSON representation of 405 error
